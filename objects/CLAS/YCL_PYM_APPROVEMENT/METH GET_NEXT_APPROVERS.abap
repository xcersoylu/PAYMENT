  METHOD get_next_approvers.
    SELECT approver~approvergroupsequence , approver~approver , userinfo~defaultemailaddress AS email ,  userinfo~personfullname AS fullname
       FROM ypym_t_approve AS approver INNER JOIN i_businessuservh AS userinfo ON userinfo~userid = approver~approver
       WHERE approver~paymentnumber = @iv_paymentnumber
         AND approver~approvergroupsequence = @iv_approvergroupsequence
